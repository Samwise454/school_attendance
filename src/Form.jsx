...
const Form = () => {
    ...
    useEffect(() => {
        ...
        const getStaffData = async () => {
            try {
                ...
                if (response.status === 200) {
                    ...
                    setStaffNum(response.data.length);
                }
            } catch (err) {
        ...
        const updateMondayCountdown = () => {
            ...
            target.setHours(23, 0, 0, 0);//increase attendance closure from here

            const distance = target - now;

            // Logic: Only run if it's Monday AND time is between 07:00:00 and 09:59:59
            if (day === 6 && hour >= 7 && hour < 23) {//to extend timer, change the hour check value
